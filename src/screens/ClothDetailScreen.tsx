...
import { 
  View, Text, Image, TouchableOpacity, ScrollView, Alert, ActivityIndicator, 
  TextInput, SafeAreaView, Platform, StatusBar, KeyboardAvoidingView, DeviceEventEmitter 
} from 'react-native';
...
import CoordLoadingModal from '../components/CoordLoadingModal'; // ★追加
...
export default function ClothDetailScreen({ route, navigation }: any) {
  ...
  // コーデ生成用ステート
  const [showLoading, setShowLoading] = useState(false); // ★追加
  const [coordResult, setCoordResult] = useState(null);
  const [showResult, setShowResult] = useState(false);

  const handleDelete = () => {
    Alert.alert("確認", "この服を削除しますか？", [
      ...
      { 
        ...
        onPress: async () => {
          ...
          try {
            ...
            Alert.alert("完了", "削除しました", [{ text: "OK", onPress: () => navigation.goBack() }]);
          } catch (e) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const userId = await getUserId();
      if (!userId) throw new Error("User ID not found");
      await api.updateCloth({ userId: userId, clothId: cloth.clothId, ...editData });
      DeviceEventEmitter.emit('CLOTHES_UPDATED');
      Alert.alert("完了", "情報を更新しました", [{ text: "OK", onPress: () => navigation.goBack() }]);
    } catch (e) {
  ...
  const handleCreateCoord = () => {
    ...
    Alert.alert(
      ...
      [
        { text: "キャンセル", style: "cancel" },
        { text: "作成する", onPress: () => setShowLoading(true) } // モーダル開く
      ]
    );
  };
  ...
  return (
    <View style={tw`flex-1 bg-white`}>
      <View style={tw`bg-[#00255C]`}>
        <SafeAreaView>
          <View style={[tw`flex-row items-center justify-between px-4 pb-4`, Platform.OS === 'android' ? { paddingTop: StatusBar.currentHeight || 24 } : { paddingTop: 10 }]}>
            ...
            <Text style={tw`text-white text-xl font-bold`}>{isEditing ? "編集" : "アイテム詳細"}</Text>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={tw`flex-1`}>
        <ScrollView contentContainerStyle={tw`p-6 pb-40`}>
          <View style={tw`items-center mb-8`}>
            <Image source={{ uri: cloth.imageUrl }} style={[tw`w-full h-80 rounded-2xl bg-gray-100`, { resizeMode: 'contain' }]} />
          </View>

          {!isEditing && (
            <TouchableOpacity onPress={handleCreateCoord} style={tw`mb-8 bg-white border-2 border-[#00255C] py-4 rounded-xl flex-row justify-center items-center shadow-sm`}>
          ...
          {isEditing ? (
            <View style={tw`gap-4`}>
              ...
              <View>
                ...
                <TextInput value={editData.description} onChangeText={(t) => setEditData({...editData, description: t})} multiline style={tw`bg-gray-50 border border-gray-200 rounded-lg p-3 text-base h-24`} />
              </View>
            </View>
          ) : (
            <View style={tw`gap-4`}>
      ...
      <View style={tw`absolute bottom-0 w-full p-6 bg-white border-t border-gray-100 flex-row gap-4`}>
        {isEditing ? (
          <TouchableOpacity onPress={handleUpdate} disabled={isProcessing} style={tw`flex-1 bg-[#00255C] py-4 rounded-full flex-row justify-center items-center shadow-lg`}>
            {isProcessing ? <ActivityIndicator color="white" /> : <Text style={tw`text-white text-center text-lg font-bold`}>更新する</Text>}
          </TouchableOpacity>
        ) : (
          <TouchableOpacity onPress={handleDelete} disabled={isProcessing} style={tw`flex-1 bg-red-50 py-4 rounded-full flex-row justify-center items-center border border-red-100`}>
            {isProcessing ? <ActivityIndicator color="#ef4444" /> : (
              <>
      ...
      <CoordLoadingModal 
        ...
        anchorClothId={cloth.clothId} // 固定IDを渡す
        onComplete={handleComplete} 
      />

      <ResultScreen visible={showResult} onClose={() => setShowResult(false)} resultData={coordResult} />
    </View>
  );
}
